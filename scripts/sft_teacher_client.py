...
- request.base_command_text에 actor unitId나 target unitId가 하나라도 명시되어 있으면, 새 sample은 base command의 actor unitId와 target unitId를 그대로 사용하지 않는다.
- 새 sample은 command, selected_bucket, skill_case, gold, output이 성립하는 범위 안에서 완전히 다른 actor unitId와 완전히 다른 target unitId를 사용해야 하며, 기존 actor-target 조합을 단순 패러프레이즈로 재사용하지 않는다.
...
출력 직전 자체 점검:
- 새 sample은 command, selected_bucket, skill_case, gold, output이 성립하는 범위 안에서 완전히 다른 actor unitId와 완전히 다른 target unitId를 사용해야 한다.
